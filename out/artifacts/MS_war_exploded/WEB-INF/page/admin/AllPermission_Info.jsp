...
<body>
        ...
        <div class="layui-card">
          ...
          <div class="layui-card-body">
            ...
            <div class="test-table-reload-btn" style="margin-bottom: 10px;">
              ...
              <!-- 由输入框获取关键字 -->
                <input class="layui-input" name="type" id="typeToReload" lay-verify="required" autocomplete="off">
              </div>
  ...
  layui.config({
    ...
    //console.log($("#tableToReload"));
    //方法级渲染
    table.render({
      elem: '#tableToReload'
      ,url: '${pageContext.request.contextPath}/Permission/FindAllPermission'
      ,title: '权限数据表'
      ,totalRow: true
      ,cols: [[
         {checkbox: true, LAY_CHECKED: false,fixed: true}
        ,{field:'id', title: '权限编号', width:80, sort: true,}
        ,{field:'name', title: '权限名称', width:160}
        ,{field:'type', title: '权限类型', width:160, sort: true}
        ,{field:'url', title: '访问地址', width:160}
        ,{field:'percode', title: '权限代码', width:160,}
        ,{field:'parentid', title: '父节点', width:160,}
        ,{field:'parentids', title: '父节点串', width:160,}
...
       }                      
    ...
    form.on("switch(changeAvailable)",function (data) {
      ...
      var id = data.elem["value"]; //通过传过来的值确定修改的role_id的属性是否为启用、停用
      var emvalue = (data.othis).find("em");
      if (emvalue.text() == "ON"){
        $.post("${pageContext.request.contextPath}/Permission/ChangePermissionAvailable",{id:id,available:'1'},function (data) {
          if (data == "success"){
        ...
        $.post("${pageContext.request.contextPath}/Permission/ChangePermissionAvailable",{id:id,available:'0'},function (data) {
          if (data == "success"){
...
/*         alert("查询关键字数为："+count); */ 
    ...
    /*$("#permissionlookbtn").on("click",function () {
      ...
      if(layevent == 'edit'){  //此事件改变该角色是否启用
        ...
        console.log(checkValue +","+ typeof checkValue);
          ...
          layer.open({
            ...
            ,maxmin: true
            ,offset: "rb"
            ,content: '${pageContext.request.contextPath}/Permission/ToGetPermissionInfo?id='+data.id
            // ,success:function (layero,index) {
            //   var iframe = window['layui-layer-iframe' + index];//拿到iframe元素
            //   console.log("在这里测试iframe"+iframe);
            //   iframe.child(id)//向此iframe层方法 传递参数
            // }
            ,btn: '关闭'
            ,btnAlign: 'c' //按钮居中
            ,area: ['30%','100%']
...
<%--   <a class='layui-btn  layui-btn-xs layui-btn-danger' lay-event='delete'>删除</a>--%>
   ...
   <button class="layui-btn layui-btn-normal" lay-event="look" id="permissionlookbtn">查看</button>
  </script>
</body>
</html>