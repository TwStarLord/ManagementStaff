            ...
            <div class="layui-card">
                ...
                <div class="layui-card-body">
                    ...
                    <div class="test-table-reload-btn" style="margin-bottom: 10px;">
                        ...
                        <div class="layui-inline">
                            <input class="layui-input" name="destination" id="destinationToReload" lay-verify="required" autocomplete="off" placeholder="目的地" >
                        </div>
                        <div class="layui-inline">
...
<script>
    layui.config({
        ...
        table.render({
            elem: '#tableToReload'
            ,url: '${pageContext.request.contextPath}/Chuchai/FindSelfChuchaiInfo'
            ,toolbar: true
            ,title: '员工出差信息表'
            ,totalRow: true
            ,cols: [[
                ...
                ,{field:'right', width:130, title: '操作',align:'center',toolbar:'#btndemo',fixed: 'right'}
            ]]
            ,page: true
            ,limits:[5,10,15]
            ,height: 'full-100'
        ...
        $("#batchdelete").click(function ()
        {
            ...
            if (count > 0)
            {
                parent.layer.confirm('确定删除？', { icon: 3 }, function (index)
                {
                    ...
                    $.ajax({
                        url: '${pageContext.request.contextPath}/BatchDeleteChuchai',
                        type: "post",
...
//departname jiesuanyuefen jibengongzi jiangli chengfa jiabangongzi kuanggonggongzi qingjiagongzi chuchaigongzi shijijiesuan
        ...
        table.on('tool(demo)',function(obj){
            var data = obj.data;if(obj.event == 'chuchaiCount'){
                $.ajax({ //提交出差报销的申请
                    url:"${pageContext.request.contextPath}/Chuchai/PostCountChuchaiFeeApply",
                    type:"post",
                    data:{chuchai_id:data.id},
                    success:function (data) {
                        if(data === "success"){
                            layer.msg("已提交出差报销申请!");
                        }else if (data === "fail"){
                            layer.msg("提交出差报销申请失败!");
                        }else{
                            layer.msg("未知错误!");
                        }
                    }
...
<script type="text/html" id="btndemo">
    <button class='layui-btn  layui-btn-xs layui-btn' lay-event='chuchaiCount' data-type='download'>出差报销</button>
</script>
</body>
</html>